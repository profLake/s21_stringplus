...
#include <stdarg.h>
>>>>>>> 2a847ddd6993a438f6b7997b696d854cf8de65e7
...
START_TEST(test_s21_strncpy) {
  char str[28] = "So many test/000/ f*hh..",
       ...
       // src[100] = "   /00never gonna give you up, never gonna let them
       // down...",
       // src[100] = "..." --- превышает 80 символов
      src1[10] = "", src11[2] = "", src2[9] = "/000/", src3[6] = "/0",
       src33[6] = "/0", src4[20] = "just like that";
...
START_TEST(test_s21_strncat) {
  ...
  // ck_assert_str_eq(s21_strncat(str, "GAGA, rama", 10), strncat(str1, "GAGA,
  // rama", 10));
...
START_TEST(test_s21_strcmp) {
  const char *h = "h", *gh = "GHTH", *empty = "", *empty1 = "",
             *ichni = "1234SZR", *end = "\0", *end1 = "\0";
...
START_TEST(test_s21_trim) {
  ...
  char *str = "1 aaaa  1";
  char *corr = "aaaa";
  char *new = (char *)s21_trim(str, nine);
  char *new1 = (char *)s21_trim(corr, nine);
  char *new2 = (char *)s21_trim(corr, str);
  ck_assert_str_eq(new, corr);
  free(new);
  ck_assert_str_eq(new1, corr);
  free(new1);
  ck_assert_ptr_eq(new2, s21_NULL);
  free(new2);
}
END_TEST
...
START_TEST(test_s21_insert) {
  char *q = "aaabbbccc", *k = "baz", *qk = "aaabazbbbccc", *p = " ",
       *qp = "aaab bbccc";
  char *new = (char *)s21_insert(q, k, 3);
  char *new1 = (char *)s21_insert(q, p, 4);
  char *new3 = (char *)s21_insert(q, p, 11);
...
int vatest_s21_trgt_print_tokn_double(char *target, char *token, ...) {
    va_list args;
    va_start(args, token);
    int result = s21_trgt_print_tokn_double(target, token, &args);
    va_end(args);
    return result;
}
START_TEST(test_s21_trgt_print_tokn_double)
{
    char target[500] = {0};
    char *target_right;
    ...
    right = 16;
    out = vatest_s21_trgt_print_tokn_double(target, token, 16, 2, 15.45);
    ck_assert_str_eq(target_right, target);
    ck_assert_int_eq(right, out);
...
Suite* s21_string_suite()
{
    ...
    tcase_add_test(tc_core, test_s21_trgt_print_tokn_double);
